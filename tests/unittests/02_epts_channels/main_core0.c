 ...
 *****************************************************************************/
void tc_1_defchnl_transport_send(struct rpmsg_channel *channel, int src, int dst, int send_value, int expected_value)
{
    void *mutex = NULL;
    int result = 0;
    result = env_create_mutex(&mutex, 1);
    TEST_ASSERT_MESSAGE(0 == result, "unexpected value");
    result = rpmsg_send_offchannel(channel, src, dst, &send_value, sizeof(send_value));
...
void tc_1_defchnl_transport(void)
{
    ...
    for (int j = 0; j < TC_TRANSFER_COUNT; j++)
    {
        ...
        tc_1_defchnl_transport_send((struct rpmsg_channel *)app_chnl,
                                    app_chnl->src,     // src addr
                                    app_chnl->dst,     // dst addr
                                    app_chnl->src,     // value to send
                                    app_chnl->src + 10 // expected value to receive
                                    );
    }
    ...
    for (int j = 0; j < TC_TRANSFER_COUNT; j++)
    {
        for (int i = 0; i < TC_EPT_COUNT; i++)
        {
            ...
            tc_1_defchnl_transport_send((struct rpmsg_channel *)app_chnl,
                                        ...
                                        epts[i]->addr,     // value to send
                                        epts[i]->addr + 10 // expected value to receive
                                        );
