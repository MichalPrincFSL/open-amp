...
void tc_1_send_receive(void)
{
    ...
    result = rpmsg_rtos_send(NULL, data, DATA_LEN, app_chnl->dst);
    TEST_ASSERT_MESSAGE(0 != result, "negative number");
    result = rpmsg_rtos_send(app_chnl->rp_ept, NULL, DATA_LEN, app_chnl->dst);
    TEST_ASSERT_MESSAGE(0 != result, "negative number");
    result = rpmsg_rtos_send(app_chnl->rp_ept, data, 0xFFFFFFFF, app_chnl->dst);
    TEST_ASSERT_MESSAGE(0 != result, "negative number");
...
void run_tests(void *unused)
{
    ...
    if (!result)
    {
        RUN_TEST(tc_1_send_receive, MAKE_UNITY_NUM(k_unity_rpmsg, 0));
        RUN_TEST(tc_2_send_receive, MAKE_UNITY_NUM(k_unity_rpmsg, 1));
    }
