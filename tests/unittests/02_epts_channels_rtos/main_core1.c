...
void tc_1_defchnl_1(void)
{
    ...
    for (int i = 0; i < TC_TRANSFER_COUNT; i++)
    {
        ...
        TEST_ASSERT_MESSAGE(received == i, "'rpmsg_rtos_recv' failed");
        ...
        received += 10;
        rpmsg_rtos_send(app_chnl->rp_ept, &received, sizeof(received), app_chnl->dst);
    }
...
void tc_1_defchnl_2(void)
{
    ...
    for (int i = 0; i < TC_TRANSFER_COUNT; i++)
    {
        ...
        TEST_ASSERT_MESSAGE(*received == i, "'rpmsg_rtos_recv' failed");
        TEST_ASSERT_MESSAGE(src == app_chnl->dst, "'rpmsg_rtos_recv' failed");
        TEST_ASSERT_MESSAGE(len == sizeof(int), "'rpmsg_rtos_recv' failed");
        (*received) += 10;
        rpmsg_rtos_send(app_chnl->rp_ept, received, sizeof(int), app_chnl->dst);
...
void tc_1_defchnl_3(void)
{
    ...
    for (int i = 0; i < TC_TRANSFER_COUNT; i++)
    {
        for (int j = 0; j < TC_EPT_COUNT; j++)
        {
            ...
            TEST_ASSERT_MESSAGE(received == epts[j]->addr, "'rpmsg_rtos_recv' failed");
            TEST_ASSERT_MESSAGE(src == epts[j]->addr, "'rpmsg_rtos_recv' failed");
            TEST_ASSERT_MESSAGE(len == sizeof(received), "'rpmsg_rtos_recv' failed");
            received += 10;
            rpmsg_rtos_send(epts[j], &received, sizeof(received), epts[j]->addr);
        }
    }
...
void tc_1_defchnl_4(void)
{
    ...
    for (int i = 0; i < TC_TRANSFER_COUNT; i++)
    {
        for (int j = 0; j < TC_EPT_COUNT; j++)
        {
            ...
            TEST_ASSERT_MESSAGE(len == sizeof(int), "'rpmsg_rtos_recv' failed");
            (*received) += 10;
            rpmsg_rtos_send(epts[j], received, sizeof(int), epts[j]->addr);
